...
function showRooms(){
	$.ajax({
		url: projectName+"/room/showRooms",
		type: "GET",
		dataType: "json",
...
function owlCarousel(rooms,pictures){
	...
	for(var i=first;i<=last;i++){
		...
		content += '<i><img src="'+projectName+'/views/images/rooms/'+pictures[i]+ '"  width="353.33" height="253.6" style="width:353.33px;height:253.6px" alt="offer"></i><div class="offer-content" style="width:353.33px;height:253.6px">';
		content += '<h5>'+rooms[i].rname+'</h5>';
		content +='<p>'+rooms[i].info+'</p>';
		content +='<span>'+rooms[i].price+'￥/晚起</span>';
		content += '<a href="'+projectName+'/room/showOneRoom?rid='+rooms[i].rid+'" title="Book Now" class="book-now">马上预订</a></div></div></div>';

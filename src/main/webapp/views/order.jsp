		...
		<div class="page-banner gallery-banner container-fluid no-padding">
			<div class="banner-content">
				...
				<h3>您预订的房间</h3>
					...
					<li><a href="index.html" title="入住时间">（form表单传数据到此）入住时间</a></li>							
					<li><a href="index.html" title="退房时间">入住时间</a></li>							
			...
			<div class="container">
				...
				<div style="height: 600px;" class="col-md-12 col-sm-12 col-xs-12">
						<div class="offer-box">
							...
							<i><img src="${pageContext.request.contextPath }/views/images/rooms/${picture.src }" alt="offer"></i>
							</c:forEach>
							<div style="width: 750px; height: 505px; float: right" class="offer-content">
								<input type="hidden" id="rid" value="${room.rid }" />
								<h3>${room.rname }</h3>
								<h5>${room.type }</h5>
								入住时间：<span id="start_time">${startTime }</span> &nbsp &nbsp退房时间：<span id="end_time">${endTime }</span>&nbsp&nbsp共<span id="count"></span>晚
								<p>${room.info }</p>
								<p>${room.equipment }</p>
								
								<span>${room.price }￥ /晚</span>
								<c:forEach var="picture" items="${pictures }">
									<a class="zoom" href="${pageContext.request.contextPath }/views/images/rooms/${picture.src }" title="offer"><i class="fa fa-plus"></i></a>
								</c:forEach>
								<a id='payNow' style="display: block;width: 170px; cursor:pointer" title="马上支付" class="book-now">确认订单，马上支付</a>&nbsp;&nbsp;
								<a id='payAfter' style="display: block;width: 170px; cursor:pointer" title="稍后支付" class="book-now">确认订单，稍后支付</a>
							</div>
						</div>
					</div>
